...
namespace Hullbreach.World
{
    ...
    public static class OrbitHelper
    {
        ...
        /// `field`: v = sqrt(Mu / r) tangential, counter-clockwise (rotate
        /// the outward radial direction +90 degrees). Returns zero if the
        /// index is out of range or `position` coincides with the body
        /// (undefined orbit).
        /// </summary>
        public static float2 CircularOrbitVelocity(GravityField field, int bodyIndex, float2 position)
        {
            ...
            float speed = math.sqrt(body.Mu / dist);
