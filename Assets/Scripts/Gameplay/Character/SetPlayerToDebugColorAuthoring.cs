        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (ghostLinkedEntityGroup, ghostOwner) in
                     SystemAPI.Query<DynamicBuffer<LinkedEntityGroup>, RefRO<GhostOwner>>()
                         ...
                         .WithAll<SetPlayerToDebugColor>())
            {
                foreach (var linkedEntity in ghostLinkedEntityGroup)
                {
                    var entity = linkedEntity.Value;

                    if (!hasDebugColorLookupRO.HasComponent(entity)) continue;
                    if (!urpMaterialPropertyBaseColorLookupRW.HasComponent(entity)) continue;

                    var colorRefRW = urpMaterialPropertyBaseColorLookupRW.GetRefRW(entity);
